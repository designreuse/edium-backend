package com.edium.library.policy.json;

import com.edium.library.policy.PolicyDefinition;
import com.edium.library.policy.PolicyRule;
...
public class JsonFilePolicyDefinition implements PolicyDefinition {
    ...
    private static String DEFAULT_POLICY_FILE_NAME = "policy/default-policy.json";
    ...
    private void init() {
        ...
        try {
            ...
            logger.info("[init] Checking policy file at: {}", policyFilePath);

            if (policyFilePath != null && !policyFilePath.isEmpty() && getClass().getClassLoader().getResource(policyFilePath) != null) {
                logger.info("[init] Loading policy from custom file: {}", policyFilePath);
                rulesArray = mapper.readValue(new File(getClass().getClassLoader().getResource(policyFilePath).getFile()), PolicyRule[].class);
            } else {
            ...
            this.rules = rulesArray != null ? Arrays.asList(rulesArray) : new ArrayList<>();
            logger.info("[init] Policy loaded successfully: " + this.rules.size());
